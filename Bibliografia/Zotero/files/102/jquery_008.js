...
(function ($, window, document) {
  ...
  var
    ...
    updateFields = function(option, $dk, reset) {
      ...
      if ($.isFunction(changeDkLabel)) {
          changeDkLabel($dk, label);
      }
    },
    ...
    enoughSpace = function($dk)  {
      // we always want dropdowns to go down
      return true;
    ...
    handleKeyBoardNav = function(e, $dk) {
      ...
      if (code >= keyMap.zero && code <= keyMap.z) {
        ...
        if (data.finder === null || typeof(data.finder) === 'undefined') {
      ...
      if (view.options && view.options.length) {
        for (i = 0, l = view.options.length; i < l; i++) {
          ...
          oTemplate = optionTemplate.replace('{{ value }}', $option.val())
                                    ...
                                    .replace('{{ text }}', jQuery.trim($option.html()))
          ;
