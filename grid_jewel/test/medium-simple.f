	  ...
	  do 20 i=1,1000
	    ...
	    if (firstchar.eq.'#') goto 20
          ...
          ELSE IF (LABEL=="MEDFILE") THEN
            READ(BUFFER,*,IOSTAT=IOS) MEDFILE
...
C--local variables
     ...
     &NTHICK,ys,MEDPART
      ...
      IF(MODMED)THEN
         ...
         EPSIN=EPS0*MEDPART(X4,Y4,Z4,T4)
...
20        CONTINUE
     ...
     & TPROFILE
      ...
      DOUBLE PRECISION XMAX, XMIN,STEP
      DOUBLE PRECISION TAU,INTERPOLATE
      XMAX=10.
      XMIN=-10.
      STEP=0.2
      MEDPART=INTERPOLATE(TPROFILE,X4,Y4,XMAX,XMIN,STEP)
      END

      DOUBLE PRECISION FUNCTION INTERPOLATE(TEMP,X4,Y4,XMAX,XMIN,STEP)
      IMPLICIT NONE
      DOUBLE PRECISION TEMP(100,100)
      DOUBLE PRECISION X4,Y4,XMAX,XMIN
      INTEGER I,J,N
      DOUBLE PRECISION T,U,AX,BX,AY,BY,STEP
      N=FLOOR(MAX((XMAX-XMIN+STEP)/STEP,0.))
      ...
      T=BX-I
      U=BY-J
      INTERPOLATE=(1-T)*(1-U)*TEMP(I,J)+(T)*(1-U)*TEMP(I+1,J)
     &+(1-T)*(U)*TEMP(I,J+1)+(T)*(U)*TEMP(I+1,J+1)
      END
